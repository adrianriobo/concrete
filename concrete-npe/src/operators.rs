...
/// };
/// use concrete_npe::estimate_external_product_noise;
...
/// // Compute the noise
/// let var_external_product = estimate_external_product_noise::<u64, _, _, BinaryKeyKind>(
///     dimension,
///     polynomial_size,
///     base_log,
///     l_gadget,
...
/// );
/// ```
pub fn estimate_external_product_noise<T, D1, D2, K>(
...
///     DecompositionBaseLog, DecompositionLevelCount, GlweDimension, PolynomialSize,
/// };
/// use concrete_npe::estimate_cmux_noise;
/// let dimension = GlweDimension(3);
/// let l_gadget = DecompositionLevelCount(4);
/// let base_log = DecompositionBaseLog(7);
/// let polynomial_size = PolynomialSize(1024);
...
/// // Compute the noise
/// let var_cmux = estimate_cmux_noise::<u64, _, _, _, BinaryKeyKind>(
///     dimension,
///     polynomial_size,
///     base_log,
///     l_gadget,
...
/// );
/// ```
pub fn estimate_cmux_noise<T, D1, D2, D3, K>(
    dimension: GlweDimension,
    polynomial_size: PolynomialSize,
    base_log: DecompositionBaseLog,
    l_gadget: DecompositionLevelCount,
...
where
    ...
    K: KeyDispersion,
{
    let var_external_product = estimate_external_product_noise::<T, _, _, K>(
        dimension,
        polynomial_size,
        base_log,
        l_gadget,
...
/// Return the variance of the GLWE after relinearization
/// # Example
/// ```rust
/// use concrete_commons::dispersion::Variance;
/// use concrete_commons::key_kinds::BinaryKeyKind;
...
/// Return a variance when computing a GLWE multiplication (i.e., tensor product and
/// relinearization)
/// # Example
/// ```rust
/// use concrete_commons::dispersion::Variance;
    ...
    K: KeyDispersion,
{
    ...
    Variance(res_1.get_variance() + res_2.get_variance())
}
...
/// };
/// use concrete_npe::estimate_keyswitch_noise_lwe_to_glwe_with_constant_term;
/// let lwe_mask_size = LweDimension(630);
/// let l_ks = DecompositionLevelCount(4);
/// let base_log = DecompositionBaseLog(7);
/// let dispersion_lwe = Variance(f64::powi(2., -38));
/// let dispersion_ks = Variance(f64::powi(2., -40));
/// let var_ks = estimate_keyswitch_noise_lwe_to_glwe_with_constant_term::<u64, _, _, BinaryKeyKind>(
///     lwe_mask_size,
///     dispersion_lwe,
///     dispersion_ks,
///     base_log,
...
/// );
/// ```
pub fn estimate_keyswitch_noise_lwe_to_glwe_with_constant_term<T, D1, D2, K>(
    lwe_mask_size: LweDimension,
    dispersion_lwe: D1,
    dispersion_ksk: D2,
    base_log: DecompositionBaseLog,
    ...
    K: KeyDispersion,
{
    ...
    Variance(
        lwe_mask_size.0 as f64
            * estimate_external_product_noise_with_binary_ggsw::<T, _, _, K>(
                poly_size,
                rlwe_mask_size,
                var_rlwe,
                var_rgsw,
